...
**Do:** Add/extend `node --test` coverage for invite token / requireInvite / related helpers. Null/missing header, edge strings. No production source edits. `npm run test` must pass. Summarize files + cases. Point to existing `tests/intakeAuth.test.mjs` if present and extend rather than duplicate blindly.
