...
**Git:** Chat-only by default. Optional audit file → branch `docs/bug-audit-s2-parse` from **`main`** only. Never share with Agent 1. No merge unless I say ship.

**Do:** Audit 401/429/MIME/partial_parse/document_scan, Gemini missing/503 graceful paths, unhandled async. Do not flag intentional soft-fails as High. Report only — no production rewrites. Name best Agent 1 tickets for this slice.
