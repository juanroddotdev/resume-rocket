...
Allowed paths: tests/ ; scripts/test-docx-mapping.mjs ; scripts/smoke-docx-template.mjs ; scripts/inventory-template-tags.mjs ; package.json (test wiring only)
...
**Do:** Strengthen mapping/smoke/inventory coverage (e.g. unzip buffer spot-checks if not already present). No PHI fixtures. `npm run test` / `test:release` green. Summarize what still needs human Word eyeball.
