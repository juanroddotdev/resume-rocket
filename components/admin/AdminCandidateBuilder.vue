...
const resumeFilenameDisplay = computed(() =>
  resumeFilename.value ? displayResumeFilename(resumeFilename.value) : '',
)
      ...
      >
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-0.5 flex min-w-0 items-center gap-x-2 overflow-x-auto whitespace-nowrap text-xs text-slate-500">
            ...
            <span v-else-if="isEditable" class="shrink-0">No resume uploaded</span>
            <span
            ...
            <template v-if="form.employers.length >= 1">
              ...
              >
                ...
                Employers ({{ form.employers.length }})
              </button>
            </template>
          </div>
        </div>
