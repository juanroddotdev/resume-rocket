...
- [x] **Supplemental data bucket** — [`buildSupplementalBucket()`](../utils/supplementalBucket.ts); admin **Extra details** non-modal right panel (Copy / Apply-to-snapshot)
...
- [x] **Candidates table as alternate dashboard view** — Builder | All candidates toggle on [`pages/admin.vue`](../pages/admin.vue); default builder + sidebar; full [`CandidatesTable.vue`](../components/admin/CandidatesTable.vue) in table mode; **Open in builder**, **Open candidate link**, Download DOCX, Parse QA row actions; `?view=table` query sync
...
- [x] **Remove builder footer copy-link bar** — Drop the pinned link row at the bottom of [`AdminCandidateBuilder.vue`](../components/admin/AdminCandidateBuilder.vue) (readonly URL + **Copy link**). Reclaim vertical space for the form canvas; handoff via **Open candidate link** in the candidates table or a future sidebar/copy affordance if needed.
...
- [x] **Employer deck in admin builder** — Slide-in employers jump panel (Extra details pattern); amber dot when name/EMR incomplete. **Temporarily all cards open** (`deck-mode="multi"`) — sticky headers on scroll kept; single-open accordion expand-in-place deferred (see backlog below).
- [ ] **Employer accordion expand-in-place** — Restore single-open accordion without scroll-jump on header click: expand the selected card in place; sticky header only engages when the user scrolls that card to the chrome. Keep jump drawer + gap-review scroll-into-view. Touches [`HospitalAutocomplete.vue`](../components/intake/HospitalAutocomplete.vue) + [`EmployerCard.vue`](../components/intake/EmployerCard.vue) (admin + intake).
...
- [x] **Hospital name = facility combobox (PR A)** — [`FacilityNameCombobox.vue`](../components/intake/FacilityNameCombobox.vue); plan: [HOSPITAL-NAME-COMBOBOX-PLAN.md](./HOSPITAL-NAME-COMBOBOX-PLAN.md) (PR B cleanup / PR C suggestions polish remain)
