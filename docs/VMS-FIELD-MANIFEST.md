...
| `compact_license_status` | `candidates.compact_license_status` *(planned)* | Planned | 3+ — Summary | Yes | Placeholder |
...
| `total_years_nursing_experience` | `candidates.years_nursing_experience` *(planned)* | Planned | 3+ — Summary | Yes | Placeholder |
| `average_patient_ratios` | `candidates.average_patient_ratios` *(planned)* | Planned | 3+ / employer detail | Yes | Placeholder |
| `specialized_medical_equipment` | `candidates.specialized_medical_equipment` *(planned)* | Planned | 3+ — Summary | Yes | Placeholder |
...
| Template tag | DB / JSON path | Parse (Gemini) | Wizard step | Required | Status |
|--------------|----------------|----------------|-------------|----------|--------|
| `education_degree` | `education[].degree` *(planned)* | Planned | 3+ — Education | Yes | Placeholder |
| `education_school_name` | `education[].school` *(planned)* | Planned | 3+ — Education | Yes | Placeholder |
| `education_graduation_year` | `education[].graduationYear` *(planned)* | Planned | 3+ — Education | Yes | Placeholder |
...
| `experience_employment_type` | `employers[].employmentType` *(planned)* | Planned | 2 — Employer card | Yes | Placeholder |
| `experience_unit_bed_count` | `employers[].unitBedCount` *(planned)* | Planned | 2 — Employer detail | No | Placeholder |
...
| `experience_patient_scope` | `employers[].patientScope` *(planned)* | Planned | 2 — Employer detail | Yes | Placeholder |
| `experience_average_daily_patients` | `employers[].avgDailyPatients` *(planned)* | Planned | 2 — Employer detail | No | Placeholder |
| `experience_patient_acuity_level` | `employers[].patientAcuity` *(planned)* | Planned | 2 — Employer detail | Yes | Placeholder |
| `{#experience_floated_units_list}{.}{/experience_floated_units_list}` | `employers[].floatedUnits[]` *(planned)* | Planned | 2 — Employer detail | No | Placeholder |
| `{#experience_equipment_procedures_list}{.}{/experience_equipment_procedures_list}` | `employers[].equipmentProcedures[]` *(planned)* | Planned | 2 — Employer detail | No | Placeholder |
| `{#experience_highlights}{.}{/experience_highlights}` | `employers[].highlights[]` *(planned)* | Planned | 2 — Employer detail | Yes | Placeholder |
