/**
 * Smoke test mapClaudeResumeJson (no API call).
 * Usage: node scripts/test-claude-parse-map.mjs
 */
import { mapClaudeResumeJson } from '../server/utils/claudeShared.ts'

const { resume: sample, audit } = mapClaudeResumeJson(
  {
    first_name: 'Jane',
    last_name: 'Doe',
...
const checks = [
  ...
  sample.education?.[0]?.city === 'Austin',
  sample.education?.[0]?.state === 'TX',
  sample.education?.[0]?.graduationMonth === '05',
  sample.education?.[0]?.graduationYear === '2016',
  sample.education?.[1]?.school === 'University of Michigan',
  sample.education?.[1]?.suggestedCity === 'Ann Arbor',
  sample.education?.[1]?.suggestedState === 'MI',
  !sample.education?.[1]?.city,
...
if (checks.some(c => !c)) {
  console.error('FAIL: mapClaudeResumeJson sample mapping incomplete', sample)
  process.exit(1)
}

console.log('OK: mapClaudeResumeJson maps VMS Phase C fields')
