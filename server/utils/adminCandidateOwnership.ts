...
export async function assertAdminOwnsInvite(user: AdminUser, inviteId: string) {
  const supabase = useSupabaseAdmin()
...
export async function assertAdminOwnsCandidate(user: AdminUser, candidateId: string) {
  const supabase = useSupabaseAdmin()
  ...
  await assertAdminOwnsInvite(user, candidate.intake_invite_id as string)
}
